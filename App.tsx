...
const App: React.FC = () => {
  ...
  const handleBatchAiAnalyze = async () => {
    ...
    const keys = (config.geminiApiKeys && config.geminiApiKeys.length > 0) 
                 ...
                 : [process.env.API_KEY].filter(Boolean) as string[];
    ...
    while (queue.length > 0) {
      ...
      // If we have 1 key, standard rate limit (~2-3s). If 5 keys, we can go faster (~500ms).
      const delayMs = Math.max(500, 2500 / keys.length);
      ...
      try {
        ...
        console.error(`Analysis failed for ${item.title} with key ending in ...${currentKey.slice(-4)}`);
        if (item.retryCount < maxRetries) {
          // Re-queue for retry
          queue.push({ ...item, retryCount: item.retryCount + 1 });
        } else {
          updatedItems.push(item); // Give up, keep original
        }
      }

      processed++;
      // Note: Total might effectively increase if we count retries as ops, 
      // but for progress bar usually we just want to show original count progress
